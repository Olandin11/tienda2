...
//lee el contenido del html y extrae la informacion del curso
 function leerDatosCurso(curso){
   ...
   var nulo=JSON.parse(localStorage.getItem("Carro"));
   if (nulo==null) {
    var jObject={};
    ...
    var jObject=JSON.parse(localStorage.getItem("Carro"));
    for(i in articulosCarrito)
    {
        suma=parseInt(i) + parseInt(otra);
        jObject[suma] = articulosCarrito[i];
        console.log(suma);
    }
}
 ...
 function carritoHTML(){
     ...
     //recorre el arreglo de objetos
        for(var i = 0; i < saber; i++){
                //console.log('si entro');
                var elemento = document.createElement('tr');
                elemento.innerHTML += ("<td><img src="+ datos[i].imagen +" width='100'></td>");
                elemento.innerHTML += ("<td>" + datos[i].titulo + "</td>");
                elemento.innerHTML += ("<td>" + datos[i].precio + "</td>");
                elemento.innerHTML += ("<td>" + datos[i].cantidad + "</td>");
                elemento.innerHTML += ("<td><a href='#'' class='borrar-curso' data-id="+datos[i].id+">"+datos[i].id+"X</a></td>");
