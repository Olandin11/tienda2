...
function obtener_producto($producto_por_pagina, $conexion){
	...
	$sentencia = $conexion->prepare("SELECT SQL_CALC_FOUND_ROWS * FROM productos LIMIT {$inicio}, {$producto_por_pagina}");
...
function numero_paginas($productos_por_pagina, $conexion){
	...
	$total_prod = $conexion->prepare('SELECT FOUND_ROWS() as total');
	$total_prod->execute();
	$total_prod = $total_prod->fetch()['total'];

	//5. Calculamos el numero de paginas que habra en la paginacion
	$numero_paginas = ceil($total_prod / $productos_por_pagina);
